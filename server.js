...
async function checkTwitchStreamers(streamers) {
  ...
  try {
    ...
    return data.map(d => {
      ...
      return { streamerId: s ? s.id : null, displayName: s ? s.displayName : d.user_name, platform: 'twitch', title: d.title, viewers: d.viewer_count, url: `https://twitch.tv/${d.user_login}` };
    });
  } catch (e) { return []; }
}
async function checkYouTubeStreamers(streamers) {
  ...
  const results = await Promise.allSettled(streamers.map(async s => {
    ...
    return { streamerId: s.id, displayName: s.displayName, platform: 'youtube', title: items[0].snippet.title, viewers, url: `https://www.youtube.com/watch?v=${videoId}` };
  }));
...
async function checkKickStreamers(streamers) {
  ...
  try {
    ...
    const results = await Promise.allSettled(streamers.map(async s => {
      ...
      return { streamerId: s.id, displayName: s.displayName, platform: 'kick', title: stream.stream_title || stream.session_title || '', viewers: stream.viewer_count || 0, url: `https://kick.com/${s.handle}` };
    }));
    return results.filter(r => r.status === 'fulfilled' && r.value).map(r => r.value);
  } catch (e) { return []; }
}
function checkTikTokStreamers(streamers) {
  return streamers.filter(s => s.manualLive).map(s => ({ streamerId: s.id, displayName: s.displayName, platform: 'tiktok', title: 'بث مباشر (مُفعّل يدويًا)', viewers: 0, url: `https://www.tiktok.com/@${s.handle}/live` }));
}
let streamCache = { data: [], expiresAt: 0 };
async function getLiveStreams() {
  if (Date.now() < streamCache.expiresAt) return streamCache.data;
  const byPlatform = { youtube: [], twitch: [], kick: [], tiktok: [] };
  ...
  const tiktok = checkTikTokStreamers(byPlatform.tiktok);
  const live = [...twitch, ...youtube, ...kick, ...tiktok].sort((a, b) => b.viewers - a.viewers);
...
async function handleApi(req, res, pathname) {
  ...
  if (pathname === '/api/logout' && method === 'POST') {
    const user = currentUserFromReq(req);
    if (user) { logAndNotify(user.wlId, user.username, `تسجيل خروج من الحساب ${user.username}.`, 'logout'); saveDB(DB); }
    const sid = getCookie(req, 'sid');
  ...
  if (pathname === '/api/admin/questions' && method === 'PUT') {
    ...
    DB.questionBank = questions; saveDB(DB);
    return sendJSON(res, 200, { questions: DB.questionBank });
  }
  ...
  if (pathname === '/api/admin/branches' && method === 'PUT') {
    ...
    DB.branches = branches; saveDB(DB);
