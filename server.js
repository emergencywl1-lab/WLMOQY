...
let nacl = null;
try { nacl = require('tweetnacl'); } catch (e) { console.warn('tweetnacl is not installed — Discord slash commands will not work until it is. Make sure Render\'s Build Command is set to "npm install".'); }
...
function defaultDB() {
  return {
    ...
    branches: [], streamers: [], officials: [], socialLinks: [],
...
function createSession(wlId) {
  const sid = newToken();
  SESSIONS.set(sid, wlId);
  DB.sessions[sid] = { wlId, expiresAt: Date.now() + SESSION_TTL_MS };
  return sid;
}
...
function hydrateSessionsFromDB() {
  ...
  for (const [sid, s] of Object.entries(DB.sessions || {})) {
    if (s.expiresAt > now) SESSIONS.set(sid, s.wlId);
    else { delete DB.sessions[sid]; changed = true; }
  }
  return changed;
}
...
function cookieSecureFlag(req) {
  ...
  // process, so the browser<->edge connection is HTTPS even though this
  // app sees plain HTTP internally. Only mark the cookie Secure when we
  // can see evidence the public connection was HTTPS, so local
  // http://localhost development still works without it.
...
/* ---------------- transactional email (password reset) — via Resend, optional ---------------- */
async function sendEmail(to, subject, html) {
  if (!RESEND_API_KEY) throw new Error('RESEND_API_KEY not configured');
  const body = JSON.stringify({ from: RESEND_FROM_EMAIL, to: [to], subject, html });
  const r = await httpsRequest('https://api.resend.com/emails', { method: 'POST', headers: { Authorization: `Bearer ${RESEND_API_KEY}`, 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(body) }, body });
  if (r.status >= 400) throw new Error('Resend error: ' + JSON.stringify(r.data));
  return r.data;
}
...
const ACTIVITY_LABELS = {
  ...
  link_roblox: '🔗 ربط Roblox', unlink_roblox: '✂️ إلغاء ربط Roblox', role_change: '🛡️ تغيير صلاحية', password_reset: '🔑 إعادة تعيين كلمة المرور'
};
...
/* ================= Discord Bot (REST-only — no gateway connection needed) =================
   DMs and role-grants are plain REST calls with the bot token — no persistent
   WebSocket connection required. This is deliberately chosen because a
   free/sleeping host (like Render's free tier) would constantly disconnect
   a gateway connection; REST calls just work whenever the server is awake. */
async function discordApi(method, path, body) {
  ...
  const r = await httpsRequest(`https://discord.com/api/v10${path}`, { method, headers, body: bodyStr });
  if (r.status >= 400) throw new Error(`Discord API ${method} ${path} failed (${r.status}): ${JSON.stringify(r.data)}`);
  return r.data;
}
async function sendDiscordDM(discordUserId, content) {
  const dm = await discordApi('POST', '/users/@me/channels', { recipient_id: discordUserId });
  await discordApi('POST', `/channels/${dm.id}/messages`, { content });
}
async function grantDiscordRole(discordUserId, roleId) {
  if (!DISCORD_GUILD_ID || !roleId) return;
  await discordApi('PUT', `/guilds/${DISCORD_GUILD_ID}/members/${discordUserId}/roles/${roleId}`, null);
}
async function getGuildMember(discordUserId) {
  if (!DISCORD_GUILD_ID) return null;
  try { return await discordApi('GET', `/guilds/${DISCORD_GUILD_ID}/members/${discordUserId}`); }
  catch (e) { return null; }
}
function discordAccountCreatedAt(discordId) {
  try {
    const ms = (BigInt(discordId) >> 22n) + 1420070400000n;
    return new Date(Number(ms));
  } catch (e) { return null; }
}

const TEST_PASS_MESSAGE = (mention) => `🎉 **تهانينا!**

يسر إدارة WL Emergency إبلاغك باجتيازك **الاختبار الإلكتروني بنجاح.**
...
const TEST_FAIL_MESSAGE = (mention) => `❌ **لم يتم اجتياز الاختبار الإلكتروني**
...
/* fire-and-forget: never let a Discord hiccup break the test-submission response */
function notifyDiscordOfTestResult(user, passed) {
  if (!user.discordLinked || !user.discordId) return;
  const mention = `<@${user.discordId}>`;
  const message = passed ? TEST_PASS_MESSAGE(mention) : TEST_FAIL_MESSAGE(mention);
  sendDiscordDM(user.discordId, message).catch(e => console.error('Discord DM failed:', e.message));
  if (passed && DB.settings.discordActivationRoleId) {
    grantDiscordRole(user.discordId, DB.settings.discordActivationRoleId).catch(e => console.error('Discord role grant failed:', e.message));
  }
}

/* ================= Discord slash command: player lookup (admin-only) ================= */
function verifyDiscordSignature(signatureHex, timestamp, rawBody) {
  if (!nacl) { console.error('Discord interaction rejected: tweetnacl not loaded (check Render Build Command is "npm install").'); return false; }
  if (!DISCORD_PUBLIC_KEY) { console.error('Discord interaction rejected: DISCORD_PUBLIC_KEY not configured.'); return false; }
  try {
    ...
    if (!ok) console.error('Discord interaction rejected: signature did not verify (wrong DISCORD_PUBLIC_KEY?).');
    return ok;
  } catch (e) { console.error('Discord interaction rejected: error during verification:', e.message); return false; }
}
...
async function registerPlayerInfoCommand() {
  if (!DISCORD_BOT_TOKEN || !DISCORD_APPLICATION_ID || !DISCORD_GUILD_ID) return;
  try {
    await discordApi('PUT', `/applications/${DISCORD_APPLICATION_ID}/guilds/${DISCORD_GUILD_ID}/commands`, [{
      name: 'player-info',
      name_localizations: { ar: 'معلومات' },
      description: 'Look up a WL member by Roblox or Discord username (staff only)',
      description_localizations: { ar: 'ابحث عن معلومات عضو بمعرف Roblox أو Discord (للإدارة فقط)' },
      options: [{ type: 3, name: 'query', name_localizations: { ar: 'بحث' }, description: 'Roblox or Discord username/ID', required: true }]
    }]);
    console.log('Discord slash command registered.');
  } catch (e) { console.error('Failed to register Discord slash command:', e.message); }
}
function formatPlayerInfoEmbed(user, guildMember) {
  const discordAge = user.discordId ? discordAccountCreatedAt(user.discordId) : null;
  const joinedGuild = guildMember && guildMember.joined_at ? new Date(guildMember.joined_at) : null;
  const attempts = user.testAttempts || [];
  const best = attempts.reduce((a, c) => (!a || c.percent > a.percent) ? c : a, null);
  return {
    embeds: [{
      title: `معلومات العضو: ${user.username}`,
      color: 0x8b5cf6,
      fields: [
        { name: 'WL ID', value: user.wlId, inline: true },
        { name: 'الصلاحية', value: user.role, inline: true },
        { name: 'تاريخ التسجيل بالموقع', value: user.createdAt, inline: true },
        { name: 'Discord', value: user.discordLinked ? `✅ ${user.discordUsername} (${user.discordId})` : '❌ غير مرتبط', inline: false },
        { name: 'عمر حساب Discord', value: discordAge ? discordAge.toISOString().slice(0, 10) : '—', inline: true },
        { name: 'انضم لسيرفرنا بتاريخ', value: joinedGuild ? joinedGuild.toISOString().slice(0, 10) : '—', inline: true },
        { name: 'Roblox', value: user.robloxLinked ? `✅ ${user.robloxUsername} / ${user.robloxDisplayName} (${user.robloxId})` : '❌ غير مرتبط', inline: false },
        { name: 'الاختبارات', value: attempts.length ? `${attempts.length} محاولة — أفضل نتيجة ${best.percent}% (${best.passed ? 'ناجح' : 'راسب'})` : 'لا توجد محاولات' }
      ]
    }]
  };
}
...
/* ================= API routes ================= */
/* ---------------- basic rate limiting (protects login/register/reset from brute force) ---------------- */
...
async function handleApi(req, res, pathname) {
  ...
  if (pathname === '/api/register' && method === 'POST') {
    if (rateLimited('register:' + clientIp(req), 6, 15 * 60 * 1000)) return sendJSON(res, 429, { error: 'محاولات كثيرة جدًا، حاول بعد شوي' });
    ...
    const sid = createSession(wlId);
    await saveDB(DB);
    res.setHeader('Set-Cookie', `sid=${sid}; HttpOnly; Path=/; SameSite=Lax; Max-Age=${Math.floor(SESSION_TTL_MS / 1000)}${cookieSecureFlag(req)}`);
    return sendJSON(res, 200, { user: publicUser(user) });
  }

  if (pathname === '/api/login' && method === 'POST') {
    if (rateLimited('login:' + clientIp(req), 10, 10 * 60 * 1000)) return sendJSON(res, 429, { error: 'محاولات كثيرة جدًا، حاول بعد شوي' });
    ...
    const sid = createSession(user.wlId);
    await saveDB(DB);
    res.setHeader('Set-Cookie', `sid=${sid}; HttpOnly; Path=/; SameSite=Lax; Max-Age=${Math.floor(SESSION_TTL_MS / 1000)}${cookieSecureFlag(req)}`);
    return sendJSON(res, 200, { user: publicUser(user) });
  }
  ...
  if (pathname === '/api/logout' && method === 'POST') {
    const user = currentUserFromReq(req);
    if (user) { logAndNotify(user.wlId, user.username, `تسجيل خروج من الحساب ${user.username}.`, 'logout'); }
    const sid = getCookie(req, 'sid');
  ...
  if (pathname === '/api/password/forgot' && method === 'POST') {
    if (rateLimited('forgot:' + clientIp(req), 5, 15 * 60 * 1000)) return sendJSON(res, 429, { error: 'محاولات كثيرة جدًا، حاول بعد شوي' });
    const { username } = await readBody(req);
    const user = DB.users.find(u => u.username.toLowerCase() === (username || '').toLowerCase());
    if (!user) return sendJSON(res, 404, { error: 'ما فيه حساب بهذا الاسم' });
    if (!user.email) return sendJSON(res, 400, { error: 'هذا الحساب ما له بريد إلكتروني مسجل — تواصل مع الإدارة لاستعادة الوصول' });
    const code = String(Math.floor(100000 + Math.random() * 900000));
    DB.passwordResets[user.wlId] = { code, expiresAt: Date.now() + 15 * 60 * 1000 };
    await saveDB(DB);
    try {
      await sendEmail(user.email, 'رمز استعادة كلمة المرور — WL Emergency',
        `<div style="font-family:sans-serif;direction:rtl;text-align:right;"><h2>استعادة كلمة المرور</h2><p>رمز التحقق الخاص بك هو:</p><h1 style="letter-spacing:4px;">${code}</h1><p>الرمز صالح لمدة 15 دقيقة. إذا لم تطلب هذا، تجاهل الرسالة.</p></div>`);
      return sendJSON(res, 200, { ok: true });
    } catch (e) {
      console.error('Password reset email failed:', e.message);
      return sendJSON(res, 500, { error: 'تعذّر إرسال البريد — تأكد إن خدمة الإيميل مُعدّة بالإعدادات (راجع README)' });
    }
  }
  if (pathname === '/api/password/reset' && method === 'POST') {
    if (rateLimited('reset:' + clientIp(req), 10, 15 * 60 * 1000)) return sendJSON(res, 429, { error: 'محاولات كثيرة جدًا، حاول بعد شوي' });
    const { username, code, newPassword } = await readBody(req);
    const user = DB.users.find(u => u.username.toLowerCase() === (username || '').toLowerCase());
    if (!user) return sendJSON(res, 404, { error: 'ما فيه حساب بهذا الاسم' });
    const reset = DB.passwordResets[user.wlId];
    if (!reset || reset.code !== code || reset.expiresAt < Date.now()) return sendJSON(res, 400, { error: 'الرمز غير صحيح أو منتهي' });
    if (!newPassword || newPassword.length < 4) return sendJSON(res, 400, { error: 'كلمة المرور الجديدة لازم تكون 4 أحرف فأكثر' });
    const { salt, hash } = hashPassword(newPassword);
    user.passwordHash = hash; user.salt = salt;
    delete DB.passwordResets[user.wlId];
    // invalidate all other active sessions for this account as a safety measure
    for (const [sid, s] of Object.entries(DB.sessions)) { if (s.wlId === user.wlId) { SESSIONS.delete(sid); delete DB.sessions[sid]; } }
    logAndNotify(user.wlId, user.username, 'تم إعادة تعيين كلمة المرور عبر البريد الإلكتروني.', 'password_reset');
    await saveDB(DB);
    return sendJSON(res, 200, { ok: true });
  }
  ...
  const unlinkMatch = pathname.match(/^\/api\/admin\/users\/([^/]+)\/unlink\/(discord|roblox)$/);
  if (unlinkMatch && method === 'POST') {
    const admin = currentUserFromReq(req);
    if (!requireRole(admin, ['admin'])) return sendJSON(res, 403, { error: 'فقط الإدارة تقدر تلغي الربط' });
    const target = DB.users.find(u => u.wlId === unlinkMatch[1]);
    if (!target) return sendJSON(res, 404, { error: 'غير موجود' });
    const provider = unlinkMatch[2];
    target[provider + 'Linked'] = false;
    logAndNotify(target.wlId, target.username, `قام ${admin.username} بإلغاء ربط حساب ${provider === 'discord' ? 'Discord' : 'Roblox'}.`, 'unlink_' + provider);
    await saveDB(DB);
  ...
  if (pathname === '/api/social' && method === 'GET') return sendJSON(res, 200, { socialLinks: DB.socialLinks });
  ...
  if (pathname === '/api/test/submit' && method === 'POST') {
    ...
    notifyDiscordOfTestResult(user, attempt.passed);
    await saveDB(DB);
  ...
  if (pathname === '/api/admin/logs' && method === 'GET') {
    const user = currentUserFromReq(req);
    if (!requireRole(user, ['staff', 'admin'])) return sendJSON(res, 403, { error: 'غير مصرح' });
    const relevant = new Set(['login', 'logout', 'link_discord', 'unlink_discord', 'link_roblox', 'unlink_roblox', 'role_change', 'account_created', 'test_result', 'password_reset']);
    let merged = [];
  ...
  if (pathname === '/api/admin/social' && method === 'PUT') {
    const admin = currentUserFromReq(req);
    if (!requireRole(admin, ['admin'])) return sendJSON(res, 403, { error: 'غير مصرح' });
    const { socialLinks } = await readBody(req, 2e6);
    if (!Array.isArray(socialLinks)) return sendJSON(res, 400, { error: 'بيانات غير صالحة' });
    DB.socialLinks = socialLinks; await saveDB(DB);
    return sendJSON(res, 200, { socialLinks: DB.socialLinks });
  }
  ...
  if (pathname === '/api/admin/settings' && method === 'GET') {
    const user = currentUserFromReq(req);
    if (!requireRole(user, ['admin'])) return sendJSON(res, 403, { error: 'غير مصرح' });
...
async function sendInteractionFollowup(token, payload) {
  const body = JSON.stringify(payload);
  const r = await httpsRequest(`https://discord.com/api/v10/webhooks/${DISCORD_APPLICATION_ID}/${token}/messages/@original`, { method: 'PATCH', headers: { 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(body) }, body });
  if (r.status >= 400) throw new Error(`Discord followup failed (${r.status}): ${JSON.stringify(r.data)}`);
}
async function handleDiscordInteractions(req, res) {
  const signature = req.headers['x-signature-ed25519'];
  const timestamp = req.headers['x-signature-timestamp'];
  const rawBody = await readRawBody(req);
  if (!signature || !timestamp || !verifyDiscordSignature(signature, timestamp, rawBody)) {
    res.writeHead(401); return res.end('invalid request signature');
  }
  const interaction = JSON.parse(rawBody);

  if (interaction.type === 1) return sendJSON(res, 200, { type: 1 }); // PING handshake

  if (interaction.type === 2 && interaction.data && interaction.data.name === 'player-info') {
    const memberRoles = (interaction.member && interaction.member.roles) || [];
    if (!DB.settings.discordAdminRoleId || !memberRoles.includes(DB.settings.discordAdminRoleId)) {
      return sendJSON(res, 200, { type: 4, data: { content: '❌ هذا الأمر للإدارة فقط.', flags: 64 } });
    }
    // acknowledge immediately (deferred), then follow up once we've gathered the data —
    // Discord requires a response within 3 seconds, and the guild-member lookup might not make it.
    sendJSON(res, 200, { type: 5, data: { flags: 64 } });
    (async () => {
      try {
        const query = interaction.data.options.find(o => o.name === 'query').value;
        const results = searchUsers(query);
        if (!results.length) {
          await sendInteractionFollowup(interaction.token, { content: `🔍 ما فيه نتائج لـ "${query}".` });
          return;
        }
        const user = results[0];
        const guildMember = user.discordId ? await getGuildMember(user.discordId) : null;
        await sendInteractionFollowup(interaction.token, formatPlayerInfoEmbed(user, guildMember));
      } catch (e) { console.error('player-info command failed:', e.message); }
    })();
    return;
  }
  return sendJSON(res, 200, { type: 4, data: { content: 'أمر غير معروف.', flags: 64 } });
}
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    if (pathname === '/discord/interactions' && req.method === 'POST') return await handleDiscordInteractions(req, res);
...
(async () => {
  ...
  const purged = hydrateSessionsFromDB();
  if (purged) await saveDB(DB);
  console.log(`Restored ${SESSIONS.size} active session(s) from storage.`);
  registerPlayerInfoCommand();
  if (DISCORD_BOT_TOKEN) {
    console.log(`Discord bot: token configured. Slash-command verification: ${nacl ? (DISCORD_PUBLIC_KEY ? 'READY' : 'MISSING DISCORD_PUBLIC_KEY') : 'tweetnacl NOT LOADED — set Render Build Command to "npm install"'}`);
  }
