...
/* ---------------- question bank (server-side ONLY — never sent to client) ---------------- */
const QUIZ_BANK = (() => {
  const bank = [
  ...
  return bank.map((row, i) => {
    ...
    return { id: i + 1, q: row[0], options: opts, correct };
  });
})();
...
async function handleApi(req, res, pathname) {
  ...
  if (pathname === '/api/test/start' && method === 'POST') {
    ...
    const n = Math.min(DB.settings.numQuestions, QUIZ_BANK.length);
  ...
  if (pathname === '/api/streams' && method === 'GET') {
    return sendJSON(res, 200, { live: [] }); // wired up in the next stage
  }
